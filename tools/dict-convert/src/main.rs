...
//! - `pack dict|lm|glossary|model|audio`：TSV → `.qj` 容器（`dict.qj` / `lm.qj`），带名称 / 许可证 / 署名元数据，输入法与 CLI 优先加载它；
//!   `model` 把本地整句模型的三件套目录打成一个 `model.qjm`；`audio` 把归一过的 `.opus` 目录打成 `audio-en.qj`
//!   （真人录音优先、合成音补缺，同时写署名清单）
//!
