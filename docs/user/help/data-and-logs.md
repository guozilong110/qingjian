...
title: 数据与日志
order: 1
description: 青简在本机保存的文件及其内容、云联想与单词发音发送的内容、输入日志的关闭与清空、运行日志的内容。
---
...
| `dicts/` | 用户导入的词库 | 否 |
| `audio-cache/` | 已下载的单词发音（音频文件与 `index.tsv`：词、许可证、录音人、来源页） | 英文词，不含中文 |
...
缺省关闭。打开「通用」页的「读出高亮候选的英语发音」后，高亮到一个本机还没有发音的英文词时，
会向维基共享资源（Wikimedia Commons）请求一次该词的发音文件：请求内容就是那个英文词，不含拼音、不含上下文、不含中文。

- 数据直接从本机发送到维基共享资源，不经过青简。
- 下载后存在 `audio-cache/`，同一个词以后不再联网。
- 密码框等安全输入中既不发音、也不请求。
- 上游没有录音的词记在内存里，不反复请求；重启后可能再问一次。
- 「通用」页可以看到已缓存多少词、占多少空间，并随时清空。
- 录音的许可证与录音人记在 `audio-cache/index.tsv`，「关于」页的「导出发音署名清单」可导出到桌面。
